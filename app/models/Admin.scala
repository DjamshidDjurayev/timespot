...
//  )
  ...
  def findAdmin(login: String, password: String) = {
    DBase.query[Admin with Persisted].whereEqual("login", login).whereEqual("password", password).fetchOne()
  }

  def getAdminId(login: String, password: String): Unit = {
    DBase.query[Admin].whereEqual("login", login).whereEqual("password", password).fetchOneId()
  }
